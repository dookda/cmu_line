...
let setCookie = (sid) => {
    ...
    document.cookie = "s330_ustoken=" + sid + "; expires=" + now.toUTCString() + "; path=/";
}
...
let logIn = () => {
    ...
    axios.post(url + '/api/score330login', { name_en: std_id.toUpperCase(), pass }).then(async (r) => {
        console.log(r.data.std_id);
        if (r.data.data) {
            setCookie(r.data.std_id)
            location.href = "./../score330/index.html";
        }
    })
}
