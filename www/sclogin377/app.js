...
let setCookie = (sid) => {
    ...
    document.cookie = "s377_ustoken=" + sid + "; expires=" + now.toUTCString() + "; path=/";
}
...
let logIn = () => {
    ...
    axios.post(url + '/api/score377login', { name_en: std_id.toUpperCase(), pass }).then(async (r) => {
        console.log(r.data.std_id);
        if (r.data.data) {
            setCookie(r.data.std_id)
            location.href = "./../score377/index.html";
        }
    })
}
